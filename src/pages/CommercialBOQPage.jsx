...
export function CommercialBOQPage({ setRoute }) {
  ...
  const handleSaveBOQ = async () => {
    ...
    setErrorMsg('');
    try {
      if (boqData._id && !boqData._id.startsWith('draft_')) {
        await apiRequest(`/api/commercial-boq/${boqData._id}`, {
          method: 'PUT',
  ...
  const handleExportExcel = async () => {
    ...
    setIsExporting(true);
    try {
      const boqId = boqData._id;
      const downloadUrl = `/api/commercial-boq/${boqId}/export/excel`;
      const a = document.createElement('a');
      a.href = downloadUrl;
      a.download = `${boqData.fileName || 'Commercial_BOQ'}_Buildiqo_Edited.xlsx`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
    } catch (err) {
      setErrorMsg('Failed to download Excel export.');
    } finally {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const boqId = boqData._id;
      const downloadUrl = `/api/commercial-boq/${boqId}/export/pdf`;
      const a = document.createElement('a');
      a.href = downloadUrl;
      a.download = `${boqData.fileName || 'Commercial_BOQ'}_Buildiqo_Report.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
    } catch (err) {
      setErrorMsg('Failed to download PDF export.');
    } finally {
      setIsExporting(false);
    }
  };
